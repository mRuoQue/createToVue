...
const templateGitRepoUrl = "https://github.com/mRuoQue/v3Template.git";
...
export default class GenProject {
  ...
  download(gitRepoUrl, name, app_path) {
    ...
    spinner.start();
    exec(
      `git clone -b main ${gitRepoUrl} ${name}`,
      async (err, stdout, stderr) => {
        if (err) {
          spinner.fail(`下载失败: ${err}`);
          console.log(`${chalk.red("try to download local template...")}`);

          copyFile(__resolve(import__dirnmae(), "../../tpl"), app_path);
        }
